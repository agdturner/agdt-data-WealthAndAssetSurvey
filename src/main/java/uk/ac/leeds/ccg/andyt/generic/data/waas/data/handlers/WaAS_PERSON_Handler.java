...
public class WaAS_PERSON_Handler extends WaAS_Handler {
 ...
 the data are selected and the cache is written for next time.
    ...
    public WaAS_DataSubsetW1 loadDataSubsetW1(TreeSet<WaAS_W1ID> w1IDs) throws IOException {
        ...
        File cf = getFile(w);
        if (cf.exists()) {
            return (WaAS_DataSubsetW1) io.readObject(cf);
        } else {
            ...
            HashMap<Data_CollectionID, PrintWriter> cPWs = initPWs(r.cFs);
            /**
            ...
            br.lines().skip(1).forEach(l -> {
                ...
                if (CASEW1 > Short.MIN_VALUE) {
                    ...
                    if (r.w1_To_c.containsKey(w1ID)) {
                        Data_CollectionID cID = r.w1_To_c.get(w1ID);
     ...
     * @throws java.io.IOException If one is encountered.
     */
    protected HashMap<Data_CollectionID, PrintWriter> initPWs(
            TreeMap<Data_CollectionID, File> cFs) throws IOException {
        HashMap<Data_CollectionID, PrintWriter> r = new HashMap<>();
        Iterator<Data_CollectionID> ite = cFs.keySet().iterator();
        while (ite.hasNext()) {
            Data_CollectionID cID = ite.next();
            //env.log("Init PW with cID " + cID);
            File f = cFs.get(cID);
            r.put(cID, io.getPrintWriter(f, false));
        }
        return r;
    }
    ...
    protected void addHeader(BufferedReader br,
            HashMap<Data_CollectionID, PrintWriter> cPWs) {
        try {
     ...
     * @param cPWs
     */
    protected void closePWs(HashMap<Data_CollectionID, PrintWriter> cPWs) {
        cPWs.keySet().stream().forEach(cID -> {
            cPWs.get(cID).close();
        });
    }
    ...
    public WaAS_DataSubsetW2 loadDataSubsetW2(WaAS_DataSubsetW1 sW1,
        ...
        File cf = getFile(w);
        if (cf.exists()) {
            return (WaAS_DataSubsetW2) io.readObject(cf);
        } else {
     ...
     */
    private void write(HashMap<WaAS_W1ID, Data_CollectionID> w1_To_c,
            WaAS_W1ID w1ID, HashMap<Data_CollectionID, PrintWriter> cPWs,
            String s) {
    ...
    public WaAS_DataSubsetW4 loadDataSubsetW4(WaAS_DataSubsetW3 sW3,
        ...
        File cf = getFile(w);
        if (cf.exists()) {
            return (WaAS_DataSubsetW4) io.readObject(cf);
        } else {
    ...
    public WaAS_DataSubsetW5 loadDataSubsetW5(WaAS_DataSubsetW4 sW4,
        ...
        File cf = getFile(w);
        if (cf.exists()) {
            return (WaAS_DataSubsetW5) io.readObject(cf);
        } else {
